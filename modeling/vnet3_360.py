...
class Vnet3_360(nn.Module):
    def __init__(self,nclasses,sync_bn=True,block_parameter=None,fcblock_parameter=None,**kwards):
        ...
        self.deconvbblock1 = DeconvBlock(**self.deconv_parameter[0],sync_bn=sync_bn)
        self.deconvbblock2 = DeconvBlock(**self.deconv_parameter[1],sync_bn=sync_bn)
        self.deconvbblock3 = DeconvBlock(**self.deconv_parameter[2],sync_bn=sync_bn)
        self.deconvbblock4 = DeconvBlock(**self.deconv_parameter[3],sync_bn=sync_bn)
        self.last_conv = nn.Conv2d(self.deconv_parameter[3]['planes'][-1],nclasses,1,stride=1,padding=0)
        self.branch1 = conv_bn_relu(self.block_parameter[0]['planes'][-1],self.block_parameter[0]['planes'][-1],1,sync_bn=sync_bn)
        self.branch2 = conv_bn_relu(self.block_parameter[1]['planes'][-1],self.block_parameter[1]['planes'][-1],1,sync_bn=sync_bn)
        self.branch3 = conv_bn_relu(self.block_parameter[2]['planes'][-1],self.block_parameter[2]['planes'][-1],1,sync_bn=sync_bn)
        self.branch4 = conv_bn_relu(self.block_parameter[3]['planes'][-1],self.block_parameter[3]['planes'][-1],1,sync_bn=sync_bn)
