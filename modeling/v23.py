...
class V23_4x(nn.Module):
    ...
    def forward(self,input):
        ...
        out = self.branch(out)
        out = torch.nn.functional.interpolate(out,size=x.data.size()[2:],mode='bilinear',align_corners=False)
        x = torch.cat([x,out],dim=1)
        x = self.last_conv2(x)
        x = torch.nn.functional.interpolate(x,size=input.data.size()[2:],mode='bilinear',align_corners=False)
        return x
