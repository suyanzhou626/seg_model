...
class VnetMloss(nn.Module):
    def __init__(self,args,block_parameter=None,fcblock_parameter=None,**kwards):
        ...
        self.semantic_conv = nn.Conv2d(self.deconv_parameter[3]['planes'][-1],2,1,stride=1,padding=0)
    ...
    def forward(self,input):
        ...
        temp_y = torch.argmax(y,dim=1,keepdim=True).type_as(x)
        x = x * temp_y
        x = self.last_conv(x)
