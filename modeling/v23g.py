...
def conv_bn_relu(inplane,outplane,k_size,stride=1,pad=0,dilation=1):
    block = nn.Sequential(OrderedDict([('conv',Conv2d(inplane,outplane,k_size,stride=stride,padding=pad,dilation=dilation)),
...
class V23_G(nn.Module):
    def __init__(self,args,block_parameter=None,fcblock_parameter=None,**kwards):
        ...
        self.last_conv1 = Conv2d(self.fcblock_parameter['planes'][-1],args.num_classes,1)
        self.branch = conv_bn_relu(self.block_parameter[1]['planes'][-1],args.num_classes,1)
        self.last_conv2 = Conv2d(2*args.num_classes,args.num_classes,3,padding=1)

        for m in self.modules():
            classname = m.__class__.__name__
            if classname.find('Conv2d')!= -1  and 'weight' in dir(m):
